...
export async function run(browser) {
    ...
    await clearFx(page);
    await pageRegisterFx(page, '/commander/fx/priest.js');
    const s = await pageFxStats(page);
    const r = { passed: 0, failed: 0 };
    if (s.layers.preFog >= 1) { r.passed++; console.log('  ✓ preFog 图层（圣链）'); } else { r.failed++; console.log('  ✗ preFog 缺失'); }
    if (s.layers.ground >= 1) { r.passed++; console.log('  ✓ ground 图层（治愈灵光）'); } else { r.failed++; console.log('  ✗ ground 缺失'); }
