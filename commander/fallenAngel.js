...
export default {
  ...
  skills: [
      ...
      { name: '净化', desc: '士气上升或下降时切换至【堕天使·黑】，攻击力+30、暴击率+60%，每回合流失当前生命值20%', type: 'passive' }
  ],
  ...
  onMoraleChange(unit, oldMorale, newMorale, helpers) {
    ...
    if (!unit._fallen && (newMorale === 1 || newMorale === 3)) {
      ...
      logMessage(`堕天使【堕落】：${unit.camp.name}${unit.config.name}兵进入黑形态，攻击力+30、暴击率+60%`);
