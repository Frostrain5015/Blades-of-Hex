...
export function planActions(gameState, helpers, myCamp) {
    ...
    // “攻城”不是一次伤害事件，而是一笔必须兑现的交易：先指定进城者，再配护航，最后才允许破门。
    // 任务跨回合保留，避免舰队和将领每次重规划都换一个方向，真正的占领手却被留在九格之外。
    const outstandingObjectives = objectiveCities.size;
    const livingAssaultUnits = allLivingUnits.filter(unit => canCaptureCityByCombat(unit));
    const assaultCapacity = assessAssaultCapacity({
        assaultCount: livingAssaultUnits.length,
        outstandingObjectives,
        ownedCities: myCities.length,
        emptyOwnedCities: myCities.filter(city => !city.unit).length
    });
    gameState._imperatorSiegeMissions ||= {};
    const previousSiegeMission = gameState._imperatorSiegeMissions[campKey];
    let primarySiegeMission = null;
    if (!defensivePhase && primaryObjective?.isCity && primaryObjective.camp !== myCamp) {
        const sameTarget = previousSiegeMission
            && previousSiegeMission.q === primaryObjective.q
            && previousSiegeMission.r === primaryObjective.r;
        const rememberedOccupier = sameTarget
            ? livingAssaultUnits.find(unit => unit.id === previousSiegeMission.occupierId)
            : null;
        const occupier = rememberedOccupier || [...livingAssaultUnits]
            .sort((left, right) => {
                const score = unit => hexDistance(unit.tile, primaryObjective) * 20
                    + (unit.commander ? 80 : 0)
                    + (isOwnCity(unit.tile) ? 35 : 0)
                    - (unit.type === 'infantry' ? 18 : 0)
                    - (unit.hp / Math.max(1, unit.maxHp)) * 15;
                return score(left) - score(right);
            })[0] || null;
        const escortIds = occupier ? [...allLivingUnits]
            .filter(unit => unit.id !== occupier.id)
            .filter(unit => !['minister', 'necromancer', 'astrologer'].includes(unit.commander))
            .sort((left, right) => {
                const leftScore = hexDistance(left.tile, occupier.tile)
                    + hexDistance(left.tile, primaryObjective) * 0.35;
                const rightScore = hexDistance(right.tile, occupier.tile)
                    + hexDistance(right.tile, primaryObjective) * 0.35;
                return leftScore - rightScore;
            })
            .slice(0, oceanMap ? 2 : 1)
            .map(unit => unit.id) : [];
        const missionState = assessSiegeMission({
            cityOwned: primaryObjective.camp === myCamp,
            cityOccupied: Boolean(primaryObjective.unit),
            cityHp: primaryObjective.hp,
            occupierDistance: occupier ? hexDistance(occupier.tile, primaryObjective) : Infinity,
            occupierMoveRange: occupier?.config?.speed,
            escortCount: escortIds.length
        });
        primarySiegeMission = {
            q: primaryObjective.q,
            r: primaryObjective.r,
            occupierId: occupier?.id || null,
            escortIds,
            phase: missionState.phase,
            occupierEta: missionState.occupierEta,
            safeToFinishBreach: missionState.safeToFinishBreach,
            assignedRound: sameTarget
                ? previousSiegeMission.assignedRound
                : currentRound,
            lastPlannedRound: currentRound
        };
        gameState._imperatorSiegeMissions[campKey] = primarySiegeMission;
    } else {
        delete gameState._imperatorSiegeMissions[campKey];
    }
    const primaryOccupierId = primarySiegeMission?.occupierId || null;
    const primaryEscortIds = new Set(primarySiegeMission?.escortIds || []);
    const isMissionCriticalUnit = unit => !!unit
        && (unit.id === primaryOccupierId || primaryEscortIds.has(unit.id));

    // 敌方正在夺取我方/中立经济节点的近战单位，是比“最近的残血”更重要的目标。
    const enemyMissionCriticality = target => {
        if (!target?.tile || !canCaptureCityByCombat(target)) return 0;
        let priority = 0;
        for (const city of [...myCities, ...neutralCities]) {
            const distance = hexDistance(target.tile, city);
            if (distance > 3) continue;
            let value = city.camp === myCamp ? 145 : 105;
            if (city.hp <= 0) value += 75;
            if (target.tile === city) value += 70;
            value -= distance * 18;
    ...
    const telemetryEntry = {
        ...
        portThreat,
        assaultCapacity,
        siegeMission: primarySiegeMission ? { ...primarySiegeMission } : null
    };
    if (telemetry.at(-1)?.round === currentRound) {
        Object.assign(telemetry[telemetry.length - 1], telemetryEntry);
    } else {
    ...
    if (primaryObjective) {
        ...
        units.sort((a, b) => {
            if (a.id === primaryOccupierId && b.id !== primaryOccupierId) return -1;
            if (b.id === primaryOccupierId && a.id !== primaryOccupierId) return 1;
    ...
    const detachmentObjectives = new Map();
    {
        ...
        if (maySplit) {
            ...
            const assigned = new Set([
                ...(primaryOccupierId ? [primaryOccupierId] : []),
                ...primaryEscortIds
            ]);
    ...
    for (const cardId of hand) {
        ...
        if (cardId === 'lightning') {
            ...
            for (const tile of gameState.tiles) {
                ...
                score += enemyMissionCriticality(target) * rainBonus;
            ...
            const healable = allUnits
                .filter(u => u._poison || u.hp < u.maxHp * 0.4
                    || (isMissionCriticalUnit(u) && u.hp < u.maxHp * 0.70
                        && estimateDestinationThreat(u.tile, u) > 0))
                .sort((a, b) => {
                    const score = unit => poisonPressure(unit)
                        + (unit.commander ? 120 : 0)
                        + (isMissionCriticalUnit(unit) ? 180 : 0)
            ...
            for (const tile of gameState.tiles) {
                ...
                score += enemyMissionCriticality(target);
            ...
            for (const u of allUnits) {
                ...
                if (u.commander) s += 50;
                if (isMissionCriticalUnit(u)) s += 170;
            ...
            // 强行军首先用于兑现攻城任务：进城者差一步时，比让普通骑兵多走两格价值高得多。
            const exhausted = allLivingUnits.filter(u => !u.canAct
                && (isMissionCriticalUnit(u) || u.commander || u.type === 'cavalry'));
            if (exhausted.length > 0) {
                exhausted.sort((a, b) => {
                    const score = unit => (isMissionCriticalUnit(unit) ? 500 : 0)
                        + (unit.id === primaryOccupierId ? 300 : 0)
                        + (unit.config.attack || 0);
                    return score(b) - score(a);
                });
...

    {
        ...
        captureTargets.sort((left, right) => {
            const leftPrimary = left === primaryObjective ? 0 : 1;
            const rightPrimary = right === primaryObjective ? 0 : 1;
        ...
        for (const city of captureTargets) {
            ...
            if (!city.unit && city.hp <= 0) {
                const enterer = assaultUnits
                    ...
                    .sort((left, right) => {
                        if (city === primaryObjective) {
                            if (left.id === primaryOccupierId && right.id !== primaryOccupierId) return -1;
                            if (right.id === primaryOccupierId && left.id !== primaryOccupierId) return 1;
                        }
            ...
            if (city.unit && isCapturableOccupant(city.unit)) {
                const finisher = assaultUnits
                    ...
                    .sort((left, right) => {
                        ...
                        return estimateDamage(right, city.unit) - estimateDamage(left, city.unit);
                    })[0];
                if (finisher) {
    ...
    for (const unit of units) {
        ...
        for (const tile of targets) {
            ...
            // 正在建立占城链条的敌方近战必须优先截杀；这是阻止对手经济滚雪球的战役级目标。
            score += enemyMissionCriticality(target);
    ...
    for (const unit of units) {
        ...
        const wouldBreach = estimateSiegeDamage(unit, siegeTarget) >= siegeTarget.hp;
        if (wouldBreach && !canCaptureCityByCombat(unit)) {
            const assignedOccupier = siegeTarget === primaryObjective
                ? allLivingUnits.find(candidate => candidate.id === primaryOccupierId)
                : null;
            const nearestOccupier = assignedOccupier || [...livingAssaultUnits]
                .sort((left, right) =>
                    hexDistance(left.tile, siegeTarget) - hexDistance(right.tile, siegeTarget))[0];
            const completion = assessSiegeMission({
                cityOwned: false,
                cityOccupied: false,
                cityHp: 0,
                occupierDistance: nearestOccupier
                    ? hexDistance(nearestOccupier.tile, siegeTarget)
                    : Infinity,
                occupierMoveRange: nearestOccupier?.config?.speed
            });
            // 不替对手拆最后一道门：至少确保指定进城者下一次行动就能完成占领。
            if (!completion.safeToFinishBreach) continue;
        }
        actions.push({ type: 'siegeCityAttack', unitId: unit.id, tileQ: siegeTarget.q, tileR: siegeTarget.r });
        processed.add(unit.id);
    }
    ...
    // 没有空城就没有陆军产能。安全后方城市若被普通驻军长期占住，在突击兵缺口存在时主动腾位，
    // 并在本计划末尾立刻排入陆军招募；尚书及受威胁的首都不参与让位。
    if (assaultCapacity.needsProductionSlot) {
        const slotCandidates = myCities
            .filter(city => city.unit?.camp === myCamp)
            .filter(city => !city.unit.commander && !processed.has(city.unit.id))
            .filter(city => strategicThreatAt(city) < 0.28)
            .map(city => ({
                city,
                unit: city.unit,
                exits: getMovableTiles(city.unit).filter(tile =>
                    !tile.unit && !tile.isCity
                    && strategicThreatAt(tile) < 0.38)
            }))
            .filter(entry => entry.exits.length > 0)
            .sort((left, right) => {
                const leftFront = primaryObjective ? hexDistance(left.city, primaryObjective) : 99;
                const rightFront = primaryObjective ? hexDistance(right.city, primaryObjective) : 99;
                return leftFront - rightFront;
            });
        const productionSlot = slotCandidates[0];
        if (productionSlot) {
            const exit = productionSlot.exits
                .sort((left, right) => strategicThreatAt(left) - strategicThreatAt(right)
                    || (primaryObjective
                        ? hexDistance(left, primaryObjective) - hexDistance(right, primaryObjective)
                        : 0))[0];
            actions.push({
                type: 'move',
                unitId: productionSlot.unit.id,
                tileQ: exit.q,
                tileR: exit.r
            });
            rememberPlannedMove(productionSlot.unit);
            plannedVacatedRecruitCities.add(`${productionSlot.city.q},${productionSlot.city.r}`);
            reservedDestinationKeys.add(`${exit.q},${exit.r}`);
            processed.add(productionSlot.unit.id);
        }
    }
    ...
    for (const unit of units) {
        ...
        const fragileCommander = ['advisor', 'diplomat', 'engineer', 'priest', 'staller']
            .includes(commanderId);
        const retreatThreshold = fragileCommander ? 0.48 : (commanderId ? 0.38 : 0.30);
        const shouldRetreat = hpRatio < retreatThreshold
        ...
        if (shouldRetreat) {
            ...
            for (const tile of validTiles) {
                ...
                const missionAdvanceBonus = unit.id === primaryOccupierId
                    ? Math.max(0, curDist - newDist) * 42
                        + (newDist <= Math.max(1, Number(unit.config?.speed || 1)) ? 55 : 0)
                    : primaryEscortIds.has(unit.id)
                        ? Math.max(0, curDist - newDist) * 18
                        : 0;
                ...
                const commanderExposure = fragileCommander ? 1.8 : (commanderId ? 1.3 : 1);
                const missionExposure = unit.id === primaryOccupierId
                    ? (tile.isCity && tile === primaryObjective ? 0.78 : 1.18)
                    : 1;
                const threatPenalty = incomingThreat / Math.max(1, unit.maxHp)
                    * (tile.isCity && tile.camp !== myCamp ? 10 : 24)
                    * commanderExposure * missionExposure;
                ...
                const score = advanceScore + missionAdvanceBonus + captureBonus + siegeReady + nearAnyTarget +
    ...
    const needsLandRecovery = emptyOwnCities.length > 0
        && (objectivePhase === 'recover' || assaultCapacity.deficit > 0 || ownForceCount <= 2);
