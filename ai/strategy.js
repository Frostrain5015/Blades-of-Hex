...
/**
 * AI 回合的看门狗预算必须把“演出等待”与实际思考区分开来。旧的固定 18 秒上限会在大军团
 * 尚未执行完计划时直接截断回合；这里按可行动单位、重规划深度与招募/部署余量给出动态预算。
 * 它仍保留硬上限，避免真正的死锁无限占用回合。
 */
export function estimateAiTurnWatchdogMs({
    actionableUnits = 0,
    replanPasses = 1,
    ...
    const delayMs = Math.max(0, Number(presentationDelayMs) || 0);
    const extras = Math.max(0, Number(extraActions) || 0);
...
/** 重规划可补射击、占领和招募，但不能让同一单位在没有强行军时反复横移。 */
export function shouldSkipReplannedMovement(action, passIndex, movedUnitIds, forceMarchedUnitIds) {
    if (!action || action.type !== 'move' || Number(passIndex) <= 0) return false;
    if (!movedUnitIds?.has?.(action.unitId)) return false;
    return !forceMarchedUnitIds?.has?.(action.unitId);
}

/** 评估一支部队是否拥有足够的“真正能进城”的陆战占领力量。 */
export function assessAssaultCapacity({
    assaultCount = 0,
    outstandingObjectives = 0,
    ownedCities = 0,
    emptyOwnedCities = 0
} = {}) {
    const objectives = Math.max(0, Number(outstandingObjectives) || 0);
    const desired = objectives > 0 ? Math.min(4, Math.max(2, objectives + 1)) : 1;
    const available = Math.max(0, Number(assaultCount) || 0);
    const deficit = Math.max(0, desired - available);
    return Object.freeze({
        desired,
        available,
        deficit,
        needsProductionSlot: deficit > 0
            && Number(ownedCities) > 0
            && Number(emptyOwnedCities) <= 0
    });
}

/** 把围城过程显式分成接近、破城、进驻与固守四个阶段，供规划和录像遥测共用。 */
export function assessSiegeMission({
    cityOwned = false,
    cityOccupied = false,
    cityHp = 0,
    occupierDistance = Infinity,
    occupierMoveRange = 0,
    escortCount = 0
} = {}) {
    const distance = Number(occupierDistance);
    const moveRange = Math.max(1, Number(occupierMoveRange) || 1);
    const eta = Number.isFinite(distance) ? Math.max(0, Math.ceil(distance / moveRange)) : null;
    let phase = 'approach';
    if (cityOwned) phase = 'hold';
    else if (!cityOccupied && Number(cityHp) <= 0) phase = 'occupy';
    else if (Number(cityHp) > 0 && Number.isFinite(distance) && distance <= moveRange * 2) phase = 'breach';
    return Object.freeze({
        phase,
        occupierEta: eta,
        occupationReady: !cityOwned && !cityOccupied && Number(cityHp) <= 0 && eta != null && eta <= 1,
        safeToFinishBreach: eta != null && eta <= 1,
        escortCount: Math.max(0, Number(escortCount) || 0)
    });
}
