...
export const MORALE_CONFIG = (() => {
    const morale = COMBAT_BALANCE.float.morale;
    const up = { name: '士气上升', dmgBonus: 0, defBonus: 0.05, icon: '▲', badgeIcon: EMOJI.moraleBadge.up, color: '#ffd700' };
    const normal = { name: '正常', dmgBonus: 0, defBonus: 0, icon: '', badgeIcon: '', color: '#aaa', desc: '' };
    const down = { name: '士气下降', dmgBonus: 0, defBonus: -0.05, icon: '▼', badgeIcon: EMOJI.moraleBadge.down, color: '#b080e8' };
    const confused = { name: '混乱', dmgBonus: 0, defBonus: -0.20, icon: '？', badgeIcon: EMOJI.moraleBadge.confused, color: '#666' };
    up.desc = `暴击率提高${percent(morale.up.max - morale.up.min)}，防御力提高${percent(up.defBonus)}`;
    down.desc = `暴击率降低${percent(Math.abs(morale.down.max))}，防御力降低${percent(Math.abs(down.defBonus))}`;
    confused.desc = `无法行动，暴击率降低${percent(Math.abs(morale.confused.min))}，防御力降低${percent(Math.abs(confused.defBonus))}`;
